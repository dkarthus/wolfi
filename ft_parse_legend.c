...
/*   Updated: 2020/12/07 12:00:56 by dkarthus         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static int ft_parse_util(char *line, t_legend *l)
{
	int flag;
	...
	if ((line[0] == ' ' || line[0] == '1') && flag == 8)
		return (ft_parse_lvl(&line, l));
	if (line[0] == '\0')
		return (1);
	return (-1);
}
...
int ft_parse_legend(char **map, t_legend *l)
{
	...
	while (map[i])
	{
		if ((flag = ft_parse_util(map[i], l)) == -1)
		{
			ft_free_leg(l);
			return (-1);
		}
